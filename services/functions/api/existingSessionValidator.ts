...
/*
* Based on a blueprint Copyright 2015-2016 Amazon.com, Inc. or its affiliates.
*
...
export const validate: APIGatewayProxyHandlerV2 = async (event, context, callback) => {
  console.log('Method ARN: ' + event.methodArn);

  // validate the incoming token
  // and produce the principal user identifier associated with the token
  let sessionID: string = event.headers.authorization || "NOAUTH"
  let requestedUserID: string = event.pathParameters.id || "NOUSER"

  let queryParams = {
    sessionID: sessionID,
    principalID: requestedUserID
  }

  console.log(`Retrieving ${queryParams.sessionID} for user ${queryParams.principalID}`)
  // let principal = await Session.get({sessionID: `Session|${sessionId}`, principalID: {userId}})
  let principal = await Session.get(queryParams)
  console.log("Principal: ")
  console.log(principal.Item)
  let policy = createPolicy(event.methodArn, requestedUserID);
  console.log(`We found ${principal.Item?.created}`)
  if (!principal.Item) {
    console.log("No Whammy")
    policy.denyAllMethods();
    let authResponse = policy.build();
    callback(null, authResponse)
  } else { 
    policy.allowMethod(AuthPolicy.HttpVerb.GET, `/user/${requestedUserID}`);
    let authResponse = policy.build();
    console.log(`Allowing /user/${requestedUserID}+`)
...
function createPolicy(eventArn: string, principalId: string) {
  var apiOptions = {};
  var tmp = eventArn.split(':');
