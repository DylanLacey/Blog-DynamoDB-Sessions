...
import { User } from "functions/users/users";
import bcrypt from "bcryptjs"

export const doAuth: APIGatewayProxyHandlerV2 = async (event, context, callback) => {
  let principalId = event.pathParameters.username
  let apiKey = event.headers.authorization
  let principal = await verifyPrincipal(principalId, apiKey)
...
// 01GCB47NWM2XP1MGEZ4PJ8RB44

const verifyPrincipal = async (principalId, apiKey) => {
  let pepper = process.env.SPICE
  try {
    // let principal = await User.query(principalId)
    let principal = await User.query(`Username|${principalId}`, {index: "GSI1"})
    let desired_hash = principal.Items[0].apiKey

    if (await bcrypt.compare(`${apiKey}|${pepper}`, desired_hash)){
      return(principal)
    }
  } catch (e) {
 ...
 * @constructor
 */
function AuthPolicy(principal: string, awsAccountId: string, apiOptions: any) {
    /**
